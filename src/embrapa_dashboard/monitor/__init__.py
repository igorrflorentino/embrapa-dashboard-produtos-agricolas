...
* :mod:`embrapa_dashboard.monitor.state` — event model, diagnosis
  heuristic (``_diagnose``), event summarisers (``_summarize``), and the
  ETA computations. No ``rich.*`` imports — testable in isolation.
* :mod:`embrapa_dashboard.monitor.render` — Rich Panel/Table builders,
...
``from embrapa_dashboard.monitor import _foo`` line.
"""

from __future__ import annotations

from embrapa_dashboard.monitor.render import (
    _build_state_grid,
    _render,
    _render_state_cell,
    _tail_jsonl,
