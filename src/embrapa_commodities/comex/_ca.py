"""Vendored CA intermediate for the Comex Stat host.

``balanca.economia.gov.br`` serves only its leaf certificate and omits the
issuing intermediate, so Python's certifi-based verification fails with
"unable to get local issuer certificate" — even though the Sectigo *root*
(R46) is trusted. curl/Secure Transport works because it fetches the
intermediate via AIA; ``requests`` does not.

We embed the intermediate (a **public** CA certificate, not a private key) and
append it to certifi's bundle at runtime so verification succeeds
deterministically on every platform — macOS dev, Linux CI, Cloud Run —
**without disabling TLS verification**.

- subject: CN=Sectigo Public Server Authentication CA OV R36
- issuer:  CN=Sectigo Public Server Authentication Root R46  (in certifi)
- source:  http://crt.sectigo.com/SectigoPublicServerAuthenticationCAOVR36.crt
- expires: 2036-03-21 — re-vendor if the host rotates its issuing CA.
"""

# Public CA certificate (PEM). Not a secret. pragma: allowlist secret
SECTIGO_INTERMEDIATE_PEM = """\
-----BEGIN CERTIFICATE-----
