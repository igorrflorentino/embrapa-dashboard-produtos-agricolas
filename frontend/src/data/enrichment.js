...
// (Manufaturado Especializado). The worklist is /api/attributes/worklist (Gold DISTINCT
...
window.enrichment = {
  ...
  apply(onDone) {
    ...
    const tasks = [...draft.entries()].map(([id, level]) => {
      ...
      return { key: id, run: () => post('/attributes/code-level', { source, code, level, change_id }) };
    });
