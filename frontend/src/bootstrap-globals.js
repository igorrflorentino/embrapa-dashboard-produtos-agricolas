...
//
// The design-system UI modules (src/ui/) read React as a *bare global* (`const {
// useState } = React`) and reference components as `window.X` (`<window.LineChart/>`).
// In strict-mode ESM a bare identifier resolves only if it exists as a globalThis
// property — so `window.React` has to be set BEFORE any ui/ module evaluates.
...
window.React = React;
window.ReactDOM = ReactDOMClient; // the ui boot uses window.ReactDOM.createRoot
