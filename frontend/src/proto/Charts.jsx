...
function LineChart({ data, height = 200, color = 'var(--viz-1)', label = 'BRL', valueKey = 'v' }) {
  const W = 560, H = height, P = { l: 54, r: 12, t: 14, b: 28 };
            ...
            {(() => {
              ...
              return (
                <text className="axis-val tnum"
                  ...
                  style={inside ? { fill: '#fff' } : undefined}>
                  {txt}
                </text>
              );
            })()}
...
function MultiLineChart({ series, height = 260, valueKey = 'v', label = '' }) {
  const W = 720, H = height, P = { l: 54, r: 12, t: 16, b: 28 };
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="chart" preserveAspectRatio="xMidYMid meet">
      {ticks.map((t, i) => (
        <g key={i}>
          <line className="grid" x1={P.l} x2={W - P.r} y1={y(t)} y2={y(t)} />
          <text className="axis" x={P.l - 6} y={y(t) + 3} textAnchor="end">{_fmtAxisNum(t)}</text>
        </g>
      ))}
      {series.map((s, si) => {
        ...
        return (
          <g key={si}>
            ...
            <circle cx={x(s.data.length - 1)} cy={y(s.data[s.data.length - 1][valueKey])} r="3" fill={s.color} />
          </g>
        );
      })}
...
Object.assign(window, { LineChart, BarChart, Donut, MultiLineChart });
