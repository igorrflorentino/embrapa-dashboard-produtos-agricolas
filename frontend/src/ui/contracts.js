...
// @property {{y:number,v:number,q:number,q_mass:number,q_vol?:number,q_count?:number}[]} overviewTS  Annual aggregate from the snapshot (v in bi; q_* per family). NOTE: overviewTS now carries q_count too (for parity with productTS/ufData), but the DISPLAYED herd count series is still recomputed client-side in dataFilters (filtered.ts.q_count) from productTS — never read off snapshot.overviewTS, never blended across families.
...
// @property {{id:string,label:string,color:string,count:number,share:number,valueShare:(number|null)}[]} quality  Quality-flag distribution (shared taxonomy). `share` is the fraction of ROWS; `valueShare` the same fraction weighted by MONEY, and the two differ by orders of magnitude — measured on prod 2026-09-07, PEVS is 81,6% UNSCORED by rows and 0,7% by value. Showing only `share` tells the researcher two thirds of the acervo went unexamined: true of the rows, false of the subject. `valueShare` is null when the source has no monetary value at all (no base for the fraction).
// @property {{y:number,ok:number,missing_value:number,missing_quantity:number,missing_weight?:number,incomplete?:number,outlier_quantity?:number,problematic_quantity?:number,outlier_value?:number,problematic_value?:number,inferred_quantity?:number,inferred_value?:number,area_inconsistent?:number,unscored?:number}[]} [qualityTs]  Quality over coverage years. Each flag is a per-year SHARE (fraction 0–1, NOT a raw count) — the views multiply by 100 for %. Flags: OK/MISSING_VALUE/MISSING_QUANTITY/INCOMPLETE (+COMEX MISSING_WEIGHT); the OUTLIER_*/PROBLEMATIC_* tiers appear only when the dbt var enable_quality_outliers is on; INFERRED_QUANTITY/INFERRED_VALUE are reserved for a future auto-fill pipeline (accepted-but-absent, always 0 today); UNSCORED marks a row the outlier detector had no basis to examine — a measured zero, a value under the materiality floor, a year the deflator does not reach, a product under the sample floor, or (PPM herd) a stock with no price at all — kept apart from OK so "examined and cleared" is not confused with "never examined". It is usually the MAJORITY flag by rows and a rounding error by value.
// @property {Object[]} [qualityByProduct]  Per-product flag shares (keys = flag ids), RANKED by row volume. It is the banco's FULL produto list (bounded only by a payload guard), not a top-N: the readability cut belongs after the researcher's product selection, on the client. Cutting server-side meant a 110-NCM banco returned 20 rows, so selecting the 50th-largest code opened an empty panel and the view's "N de 20 produtos" caption presented 20 as the whole universe.
// @property {number[]}  [valueEraBreaks]  Years the BRL changed identity (currency reform), for the ACTIVE value column — EMPTY for every convention except nominal BRL. Read from the `historical_currency_factors` seed by the seam, which already resolved the column, so the UI never re-derives which conventions are affected: it just asks whether a break falls inside its window. Drives the refusal of "variação acumulada" between two ends in DIFFERENT currencies (1974 mil cruzeiros vs 2024 reais = +5,2e15%), which is distinct from a missing value: both ends exist, they are just not commensurable.
