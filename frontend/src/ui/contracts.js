...
// @property {{y:number,ok:number,missing_value:number,missing_quantity:number,missing_weight?:number,incomplete?:number,outlier_quantity?:number,problematic_quantity?:number,outlier_value?:number,problematic_value?:number,inferred_quantity?:number,inferred_value?:number,area_inconsistent?:number}[]} [qualityTs]  Quality over coverage years. Each flag is a per-year SHARE (fraction 0–1, NOT a raw count) — the views multiply by 100 for %. Flags: OK/MISSING_VALUE/MISSING_QUANTITY/INCOMPLETE (+COMEX MISSING_WEIGHT); the OUTLIER_*/PROBLEMATIC_* tiers appear only when the dbt var enable_quality_outliers is on; INFERRED_QUANTITY/INFERRED_VALUE are reserved for a future auto-fill pipeline (accepted-but-absent, always 0 today).
