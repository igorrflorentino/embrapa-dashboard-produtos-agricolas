...
// @property {{y:number,v:number,q:number,q_mass:number,q_vol?:number}[]} overviewTS  Annual aggregate from the snapshot (v in bi; q_* per family). NOTE: the snapshot overviewTS does NOT carry q_count — the herd's count series is computed client-side in dataFilters (filtered.ts.q_count) from productTS, never blended across families.
