{#-
    ...
    Returns a NOT EXISTS clause that EXCLUDES any Gold row whose code matches a hidden
    commodity's prefix (dim_commodity_visibility). Drop it into the WHERE of any model
...
{% macro hidden_code_predicate(source_literal, code_column) -%}
    not exists (
        ...
        where v.source = '{{ source_literal }}'
          and {{ code_column }} like v.code_prefix || '%'
    )
{%- endmacro %}
