...
def test_comex_ncm_map_accepts_8_digit() -> None:
    assert Settings(comex_ncm_codes="80012100:ok").comex_ncm_map == {"80012100": "ok"}


def test_comex_ncm_map_rejects_wrong_length() -> None:
    with pytest.raises(ValueError, match=r"COMEX NCM code '8001210' has 7 digits; expected 8"):
        _ = Settings(comex_ncm_codes="8001210:short").comex_ncm_map


def test_comex_heading_map_rejects_non_4_digit() -> None:
    with pytest.raises(ValueError, match=r"COMEX HS heading code '440' has 3 digits; expected 4"):
        _ = Settings(comex_heading_codes="440:short").comex_heading_map


def test_comex_chapter_map_rejects_non_2_digit() -> None:
    with pytest.raises(ValueError, match=r"COMEX HS chapter code '4' has 1 digits; expected 2"):
        _ = Settings(comex_chapter_codes="4:short").comex_chapter_map


def test_comex_chapter_map_accepts_2_digit() -> None:
    assert Settings(comex_chapter_codes="44:madeira").comex_chapter_map == {"44": "madeira"}
