#!/usr/bin/env python3
"""
Enterprise development environment setup with Service Account impersonation.
Supports modern GCP authentication without JSON keyfiles via:
- Service Account impersonation (primary)
- OAuth with ADC (fallback)
- JSON keyfile (legacy fallback)
"""
...
class EnterpriseSetupHelper:
    """Enterprise setup with Service Account impersonation support."""
...
"""
    ...
    def run_setup(self) -> bool:
        ...
        self.print_header(f"Enterprise Development Environment Setup ({self.os_name})")
        self.print_info("Modern setup with Service Account impersonation support")
...
def main():
    """Main entry point."""
    helper = EnterpriseSetupHelper()

    try:
        success = helper.run_setup()

        if success:
            helper.print_header("✅ Enterprise Setup Complete!")
            mode = "Service Account Impersonation (OAuth)" if helper.auth_method == "impersonation" else "Legacy Keyfile"
            ...
            print("  4. See ARCHITECTURE.md for the enterprise cloud architecture")
            print("  5. Happy coding! 🚀\n")
            sys.exit(0)
        else:
            helper.print_header("❌ Setup Failed")
            print("Please review the errors above and try again.\n")
            print("For enterprise setup help, see: ARCHITECTURE.md and IAM_SETUP.md\n")
            sys.exit(1)
