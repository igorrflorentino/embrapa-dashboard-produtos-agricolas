...
### M1 · Orphan purge keys off `codigo_produto`, but detection keys off `code_prefix`
...
- **Mechanism**: `purge_plan(banco, code)` emits `DELETE … WHERE {col} LIKE '{code}%'`, where `code` is the operator-supplied `codigo_produto` (the worklist identity, and what the status guard at `:280` forces). Orphan **detection** instead matches Gold via `g.code LIKE t.code_prefix || '%'`, and `remove_produto_catalog` deliberately tombstones with the entry's *real* `code_prefix` (with a comment explaining why). `purge_plan` never reads `code_prefix`.
- **Trigger / impact**: the catalog explicitly supports a coarse prefix (`code_prefix='12'`, `codigo_produto='1203'`). Detection flags the orphan via `LIKE '12%'`; the generated purge is `LIKE '1203%'` → **under-purges** (leaves `1201`/`1204`/… behind — the very rows that made it an orphan survive a "completed" purge), and `mark_purged` then writes a **false "purged" audit record**. The reverse (codigo broader than prefix) would over-delete. Tests only ever exercise `code == prefix`, so the divergence is uncovered.
...
- **Fix**: have `purge_plan` resolve the tombstone's actual `code_prefix` (already returned by `fetch_orphan_produtos` / available on the worklist row) and build the DELETE with `LIKE code_prefix||'%'`, mirroring detection. Keep the `[A-Za-z0-9.\-]+` sanitization; refuse to plan if the prefix can't be resolved.
...
| L8 | `fetch_orphan_produtos` cache (30s TTL) is **not** invalidated on a catalog tombstone write (only `fetch_produto_catalog` is), so the Descontinuados worklist lags read-after-write up to 30s. Feature is FROZEN; advisory. | `serving/curation.py:373-374,465-470` | also `delete_memoized` the orphan reader |
...
- **F7 `visibility_clause` LIKE over-match on `%`/`_` in `code_prefix`** — **refuted**. The write gate (`curation.py:227-231`) rejects `%`/`_` in `code_prefix` with a dedicated test, so a wildcard can never reach `dim_produto_visibility`. Same for the `hidden_code_predicate` macro variant.
