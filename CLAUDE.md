...
>
> 🔄 **Dash→React migration (2026-06, branch `claude/react-migration`).** The Dash UI is being replaced by a **React SPA + Flask REST API (`src/embrapa_commodities/webapi/`) + Plotly.js** (analytical charts with zoom/hover/pan), reusing the Design System's React prototype (`frontend/src/proto/`) over the **same serving BFF** (the `seam`/`serving` layer is unchanged — the webapi serializes it to the prototype's `contracts.js` shapes). Built + **verified live on prod BQ** across every perspective (per-banco PEVS, trade, cross-source/analytics, filters). Spec: **`PLANS/react_migration_contract_map.md`** (architecture, REST table, serializer rules, the sync-over-async gate, chart→Plotly map). Frontend: `frontend/` (Vite; `npm run dev` proxies `/api`→Flask :8000; `npm run build`→`dist`). Deploy: **`deploy/webapi/`** (3-stage image, `make webapi-deploy`) targets the **same Cloud Run service/IAP/runtime SA** as the Dash app — an in-place cutover. **The Dash app (`src/.../dashboard/`) stays live until the cutover is verified; it is removed afterward** (the UI-framework-free `seam`/`format`/`registries` relocate to `webapi/` then). Until then, both exist; the webapi imports the seam from the `dashboard` package.
