...
> - **Curadoria** (per-code industrialization editor + value-added) is built but **FROZEN** — deferred to the *Versão Futura* roadmap phase and hidden from the UI (PRs #168/#169); do NOT treat it as activatable. The backend + the gated SCD2 dbt view (`enable_curation`, default false) stay as tested scaffold, and stale deep links route to a neutral notice. To revive later: un-freeze the UI entry points + `dbt build --vars 'enable_curation: true'`. **Data-blocked** (honest in-product placeholders, not pending-build): `cross_chain`/`cross_lag` + the *regime×flow market-nature* axis need sources this repo lacks (SEFAZ inter-UF flows; monthly PEVS — PEVS is annual; the customs-procedure dimension, summed away in Silver). (Note: the *ingestion* Cloud Run **Job** under `deploy/ingestion/` is a separate batch, no-UI artifact — don't confuse it with the dashboard Service.)
