...
> - **Data-access / serving layer**: `dbt/models/serving/` (pre-aggregated marts at the chart grains), `dbt/models/core/` (conformed dims + the SCD2 curation views), and `src/embrapa_commodities/serving/` (the UI-agnostic BFF: parameterized BigQuery queries, `flask-caching`, the append-only curation writer with IAP author capture).
> - **REST API + SPA host**: `src/embrapa_commodities/webapi/` (Flask app factory; serves the built SPA **and** `/api` from one origin/IAP; `seam` composes the gateway readers, `serializers` shapes them to the prototype's `contracts.js`; `format`/`registries` are the pt-BR formatting + banco/metric/view registries — these moved here from the deleted Dash package). gunicorn entrypoint `embrapa_commodities.webapi.app:app`; extra `webapi` (flask + flask-caching + gunicorn, **no dash/plotly**).
> - **Frontend**: `frontend/` — the Design System's React/Vite prototype reused verbatim (`frontend/src/proto/`), with the synthetic data layer + SVG charts replaced by API-backed `src/data/` + Plotly.js `src/charts/` (analytical charts get zoom/hover/pan). `npm run dev` (Vite :5173, proxies `/api`→Flask :8000) · `npm run build`→`dist`.
...
> - **Curadoria** (per-code industrialization editor + value-added) is built; it needs **prod activation** — `dbt build --vars 'enable_curation: true'` (the two SCD2 views) + IAP (or `curation_dev_author`) for write-author capture. **Data-blocked** (honest in-product placeholders, not pending-build): `cross_chain`/`cross_lag` + the *regime×flow market-nature* axis need sources this repo lacks (SEFAZ inter-UF flows; monthly PEVS — PEVS is annual; the customs-procedure dimension, summed away in Silver). (Note: the *ingestion* Cloud Run **Job** under `deploy/ingestion/` is a separate batch, no-UI artifact — don't confuse it with the dashboard Service.)
